...
class TestCSVReader:
    ...
    def test_read_doublequote_false_no_escapechar_error(self):
        ...
        with pytest.raises(csv.Error, match="delimiter expected after"):
    ...
    def test_quoting_all(self):
        ...
        data = '"a","b","c"\r\n"1","2","3"'
        sio = io.StringIO(data)
        r = csv.reader(sio, quoting=csv.QUOTE_ALL) # Affects writer mainly, reader adapts
        assert list(r) == [['a', 'b', 'c'], ['1', '2', '3']]
    ...
    def test_quoting_none_no_escapechar_error(self):
        ...
        assert list(r_err) == [['a', 'b', 'c', 'd'],['e', 'f', 'g'],['hello','world']]
    ...
    def test_different_lineterminators_if_possible(self):
        ...
        assert list(csv.reader(["a,b", "c,d"])) == [['a','b'],['c','d']] # Simulates any line ending already processed
    ...
    def test_reader_error_unexpected_chars_after_quotes_strict(self):
        ...
        with pytest.raises(csv.Error, match="'b' found after quoted field" ): # Or similar, based on exact error msg
            list(csv.reader(sio, strict=True))
    ...
    def test_field_size_limit_reader(self):
        ...
        try:
            ...
            with pytest.raises(csv.Error, match=f"field larger than field limit \\({limit}\\)"):
...
class TestCSVWriter:
    ...
    def test_quoting_nonnumeric_writer(self):
        ...
        w.writerow(['a', 1, 2.0, 'b,c', None, True]) # True is non-numeric by this logic
        ...
        w2.writerow([1.0, "2;0", "text"]) # "2;0" is a string, not numeric for isinstance check
    ...
    def test_quoting_none_writer_no_escapechar_error(self):
        ...
        with pytest.raises(csv.Error, match="delimiter or quotechar found in field, but escapechar is not set"):
    ...
    def test_writer_doublequote_false_no_escapechar_error(self):
        sio = io.StringIO()
        w = csv.writer(sio, doublequote=False, quoting=csv.QUOTE_ALL) # escapechar is None by default
        with pytest.raises(csv.Error, match="quotechar found in field, but no escape mechanism"):
            w.writerow(['a"b'])
...
class TestCSVDialect:
    def test_register_get_list_unregister_dialect(self, dialect_cleanup): # Use fixture
        ...
        dialect_cleanup("test_custom", delimiter=';', quotechar="'", quoting=csv.QUOTE_ALL)
    ...
    def test_register_with_dialect_instance(self, dialect_cleanup):
        custom_dialect = csv.Dialect(delimiter='|', quoting=csv.QUOTE_NONE, escapechar='!')
    ...
    def test_register_with_base_dialect_and_fmtparams(self, dialect_cleanup):
        ...
        dialect_cleanup("derived_fmt", dialect="base_for_fmt", quotechar='"', skipinitialspace=True)
    ...
    def test_dialect_properties_validation(self):
        with pytest.raises(TypeError, match="delimiter must be a single character string"):
    ...
    def test_use_custom_dialect_with_reader_writer(self, dialect_cleanup):
        dialect_cleanup("myio", delimiter=':', lineterminator='!', quotechar="'", quoting=csv.QUOTE_ALL)
...
class TestCSVSniffer:
    ...
    def test_has_header(self):
        ...
        assert sniffer.has_header(sample_no_header_text) is False # Heuristic might fail here
...
This initial structure covers a good portion of the test cases. I've added a `dialect_cleanup` fixture for managing dialect registration in tests.
I've started with `TestCSVReader`, `TestCSVWriter`, `TestCSVDialect`, `TestCSVSniffer`, and `TestCSVGeneral`.

I'll need to continue populating these classes with more detailed tests as per the requirements.
The `test_embedded_newlines_in_quoted_fields` highlights a known limitation of the current Python-based reader if it doesn't handle multi-line fetching from file streams. The test confirms it works with pre-split logical rows.

The `Sniffer` tests are for its basic functionality as implemented. `has_header` relies on heuristics which may not be perfect but the test covers some scenarios.

The `field_size_limit` tests for the reader are included. Writer doesn't typically check this limit itself, but relies on underlying stream or system limits.

This is a substantial file. I'll submit this first part.
